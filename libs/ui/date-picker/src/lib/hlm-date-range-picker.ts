...
@Component({
	...
	providers: [HLM_DATE_RANGE_PICKER_VALUE_ACCESSOR, provideIcons({ lucideCalendarRange })],
	...
	template: `
		...
		>
			<button
				...
				hlmPopoverTrigger
			>
				<ng-icon hlm size="sm" name="lucideCalendarRange" class="text-muted-foreground" />
				<span class="truncate">
...
export class HlmDateRangePicker<T> implements ControlValueAccessor {
	...
	protected readonly _computedClass = computed(() =>
		hlm(
			'ring-offset-background border-input bg-background hover:bg-accent dark:bg-input/30 dark:hover:bg-input/50 hover:text-accent-foreground inline-flex h-9 w-[280px] cursor-default items-center justify-start gap-2 rounded-md border px-3 py-2 text-left text-sm font-normal whitespace-nowrap transition-all disabled:pointer-events-none disabled:opacity-50',
